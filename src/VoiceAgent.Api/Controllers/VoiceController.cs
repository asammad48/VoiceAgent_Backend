...
using VoiceAgent.Application.DTOs.Voice;
...
public class VoiceController(IVoiceSessionService voiceSessionService) : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<VoiceStartResponseDto>>> Start([FromBody] VoiceStartRequest request, CancellationToken ct)
    {
        ...
        var response = new VoiceStartResponseDto
        {
        ...
        return Ok(ApiResponse<VoiceStartResponseDto>.Ok(response, "Voice session started."));
    }

    [HttpPost("session/end")]
    public async Task<ActionResult<ApiResponse<bool>>> End([FromBody] VoiceEndRequest request, CancellationToken ct)
    {
        await voiceSessionService.EndSessionAsync(request.CallSessionId, ct);
        return Ok(ApiResponse<bool>.Ok(true, "Voice session ended."));
    }
